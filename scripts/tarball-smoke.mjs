...
if (version !== '0.1.5') throw new Error(`expected CLI version 0.1.5, got ${version}`);
...
const imports = run('node', ['--input-type=module', '-e', "import { MCPSNITCH_VERSION, inferProfileFromCommand, formatActionableAlert, initMcpSnitch } from 'mcpsnitch'; if (MCPSNITCH_VERSION !== '0.1.5') throw new Error('bad version export'); if (inferProfileFromCommand('npx', ['@modelcontextprotocol/server-github']) !== 'github') throw new Error('bad profile export'); if (typeof formatActionableAlert !== 'function') throw new Error('missing alert export'); if (typeof initMcpSnitch !== 'function') throw new Error('missing init export');"], { cwd: root });
