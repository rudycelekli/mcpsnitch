...
export async function startMcpServer(): Promise<void> {
  const server = new McpServer({ name: 'mcpsnitch', version: '0.1.3' }, { instructions: 'MCPSnitch audit tools for MCP tool-call observability. Long-running proxy mode is the CLI: mcpsnitch watch -- <server command>.' });
