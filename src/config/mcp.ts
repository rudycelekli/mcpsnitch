...
export function defaultMcpConfigCandidates(root = '.', includeGlobal = false): string[] {
  ...
  if (includeGlobal) candidates.push(join(homedir(), '.claude.json'));
  return candidates;
}

export function findMcpConfigPath(root = '.', explicit?: string, includeGlobal = false): string {
  if (explicit) return resolve(root, explicit);
  const candidates = defaultMcpConfigCandidates(root, includeGlobal);
  for (const path of candidates) {
    if (!existsSync(path)) continue;
    const data = readJsonFile(path);
    if (isRecord(data) && isRecord(data.mcpServers)) return path;
  }
...
export function isRemoteMcpServer(entry: McpServerEntry): boolean {
...
function wrappedInvocation(entry: McpServerEntry): { args: string[]; runIndex: number; sep: number } | undefined {
  const args = asStringArray(entry.args);
  const command = typeof entry.command === 'string' ? entry.command : '';
  const runIndex = args.indexOf('run');
  if (runIndex < 0) return undefined;
  const sep = args.indexOf('--', runIndex + 1);
  if (sep < 0 || sep >= args.length - 1) return undefined;
...
export function isWrappedMcpServer(entry: McpServerEntry): boolean {
  return !!wrappedInvocation(entry);
}

function backupPathFor(path: string, now = new Date()): string {
  const base = `${path}.bak`;
...
function backupFile(path: string, now = new Date()): string {
  const backupPath = backupPathFor(path, now);
  copyFileSync(path, backupPath);
  return backupPath;
}

function wrapperPrefix(opts: InitOptions): string[] {
  if (opts.wrapperArgs && opts.wrapperArgs.length > 0) return opts.wrapperArgs;
  const wrapperCommand = opts.wrapperCommand ?? 'npx';
  if (wrapperCommand === 'npx') return ['-y', opts.wrapperPackage ?? DEFAULT_WRAPPER_PACKAGE];
  return [];
}

function makeWrappedEntry(name: string, entry: McpServerEntry, profile: string, root: string, profileConfigPath: string, opts: InitOptions): McpServerEntry {
...
export function buildProfilesConfig(config: McpConfig, generatedAt = new Date().toISOString()): ProfilesConfig {
  const profiles: Record<string, string> = {};
  const servers: ProfilesConfigServer[] = [];
...
export function readProfileSpecForServer(path: string | undefined, serverName: string | undefined, root?: string): string | undefined {
  if (!path || !serverName) return undefined;
  const resolved = resolve(root ?? '.', path);
  if (!existsSync(resolved)) return undefined;
  const data = readJsonFile(resolved);
  if (!isRecord(data) || !isRecord(data.profiles)) return undefined;
  const value = data.profiles[serverName];
  if (typeof value !== 'string' || !value.trim()) return undefined;
  const spec = value.trim();
  if (spec === 'auto' || BUILTIN_PROFILES[spec] || isAbsolute(spec)) return spec;
  return resolve(root ?? dirname(resolved), spec);
}

export function initMcpSnitch(opts: InitOptions = {}): InitResult {
  const root = resolve(opts.root ?? '.');
  const configPath = findMcpConfigPath(root, opts.configPath, !!opts.includeGlobal);
  const config = readJsonFile(configPath);
  if (!isRecord(config) || !isRecord(config.mcpServers)) throw new Error(`invalid MCP config ${configPath}: expected top-level mcpServers object`);
  ...
  const nextConfig: McpConfig = { ...mcpConfig, mcpServers: { ...mcpConfig.mcpServers } };
...
function unwrapEntry(entry: McpServerEntry): McpServerEntry | undefined {
...
export function uninitMcpSnitch(opts: UninitOptions = {}): UninitResult {
  const root = resolve(opts.root ?? '.');
  const configPath = findMcpConfigPath(root, opts.configPath, !!opts.includeGlobal);
  const config = readJsonFile(configPath);
  if (!isRecord(config) || !isRecord(config.mcpServers)) throw new Error(`invalid MCP config ${configPath}: expected top-level mcpServers object`);
  const nextConfig: McpConfig = { ...(config as McpConfig), mcpServers: { ...(config as McpConfig).mcpServers } };
