...
export async function startMcpServer() {
    const server = new McpServer({ name: 'mcpsnitch', version: MCPSNITCH_VERSION }, { instructions: 'MCPSnitch audit tools for MCP tool-call observability. Long-running proxy mode is the CLI: mcpsnitch run -- <server command>.' });
    ...
    server.tool('snitch_report', 'Return the per-session audit report: tool calls, cost estimate, permission scopes, data-flow hints, and anomalies.', { root: z.string().optional() }, async ({ root }) => failOpen('run snitch_analyze or mcpsnitch run/watch first', () => summarize(loadEvents(root))));
    server.tool('snitch_verify_log', 'Verify the hash-chained JSONL audit log for tampering.', { root: z.string().optional() }, async ({ root }) => failOpen('run snitch_analyze or mcpsnitch run/watch first', () => verifyLog(root)));
