...
function collect(value, previous = []) { previous.push(value); return previous; }
...
program.command('run').description('Silent-when-clean stdio wrapper: mcpsnitch run -- <mcp-server-command> [args...]').option('--root <path>', 'workspace root', '.').option('--server-name <name>', 'MCP server name for alert context and profile-config lookup').option('--profile-config <path>', 'editable .mcpsnitch/profiles.json mapping from mcpsnitch init').option('--profile <name-or-path>', 'expected behavior profile name/path, or auto-match known server commands', 'auto').option('--observer-interval <ms>', 'process observer sampling interval', (v) => Number(v)).option('--no-process-observer', 'disable best-effort lsof process observation and record a self-report-only downgrade').option('--verbose', 'print info-level observer status instead of staying silent when clean').allowUnknownOption(true).argument('[cmd...]').action(async (cmd, o) => { const sep = cmd; if (!sep.length)
    fail(false, 2, 'missing command: mcpsnitch run -- <server>'); const code = await watchStdio({ root: o.root, command: sep[0], args: sep.slice(1), processObserver: o.processObserver, profile: o.profile, serverName: o.serverName, profileConfigPath: o.profileConfig, observerIntervalMs: o.observerInterval, quiet: !o.verbose, alertMode: 'process' }); process.exit(code); });
program.command('watch').description('Transparent diagnostic stdio proxy: mcpsnitch watch -- <mcp-server-command> [args...]').option('--root <path>', 'workspace root', '.').option('--server-name <name>', 'MCP server name for alert context and profile-config lookup').option('--profile-config <path>', 'editable .mcpsnitch/profiles.json mapping from mcpsnitch init').option('--profile <name-or-path>', 'expected behavior profile name/path, or auto-match known server commands', 'auto').option('--observer-interval <ms>', 'process observer sampling interval', (v) => Number(v)).option('--no-process-observer', 'disable best-effort lsof process observation and record a self-report-only downgrade').option('--quiet', 'suppress info-level observer status; medium/high alerts still print').allowUnknownOption(true).argument('[cmd...]').action(async (cmd, o) => { const sep = cmd; if (!sep.length)
    fail(false, 2, 'missing command: mcpsnitch watch -- <server>'); const code = await watchStdio({ root: o.root, command: sep[0], args: sep.slice(1), processObserver: o.processObserver, profile: o.profile, serverName: o.serverName, profileConfigPath: o.profileConfig, observerIntervalMs: o.observerInterval, quiet: !!o.quiet, alertMode: 'all' }); process.exit(code); });
program.command('init')
    ...
    .action((o) => { try {
    const result = initMcpSnitch({ root: o.root, configPath: o.config, profileConfigPath: o.profileConfig, dryRun: !!o.dryRun, wrapperCommand: o.wrapperCommand, wrapperArgs: o.wrapperArg, wrapperPackage: o.wrapperPackage, includeGlobal: !!o.global });
    out(o.json, result, () => { for (const msg of result.messages)
        console.log(msg); console.log(`profiles: ${result.profileConfigPath}`); if (!result.changed)
        console.log('no stdio MCP config changes needed'); });
}
catch (e) {
    fail(o.json, 2, e.message);
} });
program.command('uninit')
    ...
    .option('--json', 'machine-readable output')
    .action((o) => { try {
    const result = uninitMcpSnitch({ root: o.root, configPath: o.config, dryRun: !!o.dryRun, includeGlobal: !!o.global });
    out(o.json, result, () => { for (const msg of result.messages)
        console.log(msg); });
}
catch (e) {
