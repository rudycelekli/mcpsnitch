...
export function loadEvents(root = '.') {
    const p = paths(root);
    return rawLines(p.log).map((line, i) => {
        try {
            ...
            throw new Error(`audit log parse error at line ${i + 1}: ${e.message}`);
        }
    });
}
export function verifyLog(root = '.') {
    const p = paths(root);
    const lines = rawLines(p.log);
    for (let i = 0; i < lines.length; i++) {
        let event;
        try {
            event = AuditEventSchema.parse(JSON.parse(lines[i]));
        }
        catch (e) {
            return { ok: false, entries: lines.length, chainValid: false, firstBreak: { seq: i, reason: `parse error: ${e.message}` } };
        }
        if (event.seq !== i)
            return { ok: false, entries: lines.length, chainValid: false, firstBreak: { seq: i, reason: `seq mismatch: got ${event.seq}` } };
        const expectedPrev = i === 0 ? '' : sha256Hex(lines[i - 1]);
        if (event.prevHash !== expectedPrev)
            return { ok: false, entries: lines.length, chainValid: false, firstBreak: { seq: i, reason: 'prevHash mismatch' } };
        const { hash, ...withoutHash } = event;
        if (hash !== sha256Hex(stableJson(withoutHash)))
            return { ok: false, entries: lines.length, chainValid: false, firstBreak: { seq: i, reason: 'entry hash mismatch' } };
    }
    return { ok: true, entries: lines.length, chainValid: true };
}
export function summarize(events, sessionId = events[0]?.sessionId ?? 'none') {
    const findings = events.flatMap((e) => e.findings);
    const byTool = {};
